...
node *nyxh_pre_add(node *state,node *obj,node *block,node *parameters)//TODO
{
  ...
  {
    char *num=convert_long_to_string(node_GetSint32(real_value2));
...
node *nyxh_str(node *state,node *obj,node *block,node *parameters)
{
  ...
  if(node_GetType(real_value2)==NODE_TYPE_SINT32)
  {
    char *str = convert_long_to_string(node_GetSint32(real_value2));
    node_SetString(real_value,str);
    free(str);
  }
  else if(node_GetType(real_value2)==NODE_TYPE_DOUBLE)
  {
    char *str = convert_double_to_string(node_GetDouble(real_value2));
    node_SetString(real_value,str);
    free(str);
  }
  else if(node_GetType(real_value2)==NODE_TYPE_STRING)
  ...
  while(i<str_len)
  {
    ...
    while(spi<splitter_len)
    {
      ...
      if(c==s)
      {
        ...
        if(tok_len)
        {
          char *tok = SubString(str,last_split,tok_len);
  ...
  if(socket_handle!=NULL && socket_type != NULL)
  {
    ...
    if(node_GetItemsNum(parameters))
    {
      ...
      { 
        ...
        char *host = node_GetString(real_nhost);
        int port = node_GetSint32(real_nport);
        char *sport = convert_long_to_string(port);
        ...
        if(r==0)
        {
          ...
          while(test!=NULL)
          {
            ...
            close(sock);
            sock = socket(test->ai_family,socket_type,test->ai_protocol);
...
node *nyxh_socket_read(node *state,node *obj,node *block,node *parameters)
{
  ...
  if(nsocket_handle!=NULL)
  {
    ...
    {
      memset(buf,0,1024);
      len = recv(socket_handle,&buf,1023,0);
      data_received+=len;
      //printf("recv len:%d\n",len);
      //fflush(stdout);
      if(len<0)
    ...
    node_SetString(real_value,ret);
    set_obj_ptr(value,"socket_handle",socket_handle);
    free(ret);
  }
  return(value);
}
...
node *nyxh_http_parse_answer(node *state,node *obj,node *block,node *parameters)
{
  ...
  while(i<answer_len)
  {
    ...
    if(c=='\r' || c == '\n')
    {
      ...
      {
        char *line = SubString(answer,last_line_start,i-last_line_start);
        //printf("line:[%s]\n",line);
        free(line);
      }
  ...
  char *body = SubString(answer,i+1,answer_len-i-1);
  //printf("body:[%s]\n",body);
